...
**Performance (2026-08-23 load-time pass):**

- `get_realtime_prices_batch()` fetches per-symbol `fast_info` **in parallel**
  (`ThreadPoolExecutor`, up to 8 workers) instead of sequentially — the
  dashboard's 4 index tickers went from 4 serial network round-trips to 1
  parallel batch. Debug `print()` lines on every realtime fetch were removed.
- `_REALTIME_CACHE_TTL` raised 5s → 15s (the live ticker fragment already
  refreshes every 30s, so 5s only caused constant refetching).

...
# Evaluate the whole Projections sheet, column F chain
for (col, row), val in sorted(_wb_sheets['Projections'].items(), key=lambda kv: kv[0][1]):
    if col == 'F' or row == 9:
        if isinstance(val, str) and val.startswith('='):
            res = eval_formula(val, _wb_sheets['Projections'])
            print(f'{col}{row}: {val} -> {res}')
        else:
            print(f'{col}{row}: {val!r}')

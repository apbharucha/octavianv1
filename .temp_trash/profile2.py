...
t0 = time.time()
print(f"[{time.time()-t0:.0f}s] from_file...", flush=True)
at = AppTest.from_file("main.py", default_timeout=240)
print(f"[{time.time()-t0:.0f}s] from_file done", flush=True)
...
t0 = time.time()
print(f"[{time.time()-t0:.0f}s] run with mocks...", flush=True)
with patch("data_sources.get_stock", side_effect=_mock_get_stock), \
    ...
    at.run()
    print(f"[{time.time()-t0:.0f}s] at.run() done, exceptions={[str(e.value) for e in at.exception]}", flush=True)

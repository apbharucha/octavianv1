...
# Find latest v10 runs per bucket (run ids starting with bucket-<bucket>-2026)
buckets = ['small', 'medium', 'huge', 'mega']
print('=== V10 FINAL TALLY ===')
tot_n = tot_ok = 0
for b in buckets:
    rows = c.execute(
        "SELECT run_id, COUNT(*) n, SUM(passed) ok FROM evaluations "
        "WHERE run_id LIKE ? GROUP BY run_id ORDER BY MAX(id) DESC LIMIT 1",
        (f'bucket-{b}-2026%',)
    ).fetchall()
    for r in rows:
        n, ok = r['n'], r['ok']
